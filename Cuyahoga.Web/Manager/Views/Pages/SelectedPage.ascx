...
<div class="taskcontainer">
	<% using (Html.BeginForm("SavePageProperties", "Pages", new { id = ViewData.Model.Id }, FormMethod.Post, new { id = "pageform" })) { %>
		...
		<p>
			...
			/<%= Html.TextBox("ShortDescription", ViewData.Model.ShortDescription, new { style = "width:90%;" })%>
		</p>
		<p>
			<label for="Culture"><%= GlobalResources.CultureLabel %></label>
			<%= Html.DropDownList("Culture", ViewData["Cultures"] as SelectList, new { style = "width:90%;" })%>
		</p>
		<p>
